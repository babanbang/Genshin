...
import { Avatar, Weapon } from "..";
import { AttrData } from "./AttrData";
import { getWeaponBuff } from "../WeaponData";
...
export class Attr extends BasePanel<GameList.Gs>{
	...
	/** 计算角色属性 */
	calcCharAttr() {
		const { char, level, promote } = this.avatar
		...
		const detailLeft = details[(lv.left + '+') as AttrDetailKeys] || details[String(lv.left) as AttrDetailKeys]
		const detailRight = details[String(lv.right) as AttrDetailKeys]
