...
defParams = ({ weapon }) => weapon.name === '赤月之形' ? { BondOfLife: 155 } : { BondOfLife: 130 }// 生命之契在此调整,请勿超过200

buffs = [{
  title: '阿蕾奇诺天赋：[buffCount]%最大生命值的生命之契,普通攻击伤害提升[aPlus]',
  data: {
    buffCount: ({ params }) => Math.min(params.BondOfLife, 200),
    aPlus: ({ talent, attr, params, cons }) => attr.atk * Math.min(params.BondOfLife, 200) / 100 * (cons >= 1 ? (talent[GsTalentType.a].get('红死之宴提升'] + 100) : talent[GsTalentType.a].get('红死之宴提升']) / 100
  }
}, {
