details = [{
  ...
  dmg: ({ talent, attr }, { basic }) => {
    const td = talent[GsTalentType.e].get('灭净三业伤害2']
  ...
  title: '六命特殊E伤害',
    cons: 6,
      dmg: ({ attr }, { basic }) => basic(attr.atk * 2.00 + attr.mastery * 4.00, GsTalentType.e)
}, {
  title: '六命特殊E伤害·蔓激化',
    cons: 6,
      dmg: ({ attr }, { basic }) => basic(attr.atk * 2.00 + attr.mastery * 4.00, GsTalentType.e, 'spread')
}]

defDmgIdx = 4
mainAttr = 'atk,mastery,cpct,cdmg'
...
buffs = [{
  ...
  data: {
    eDmg: ({ cons, talent, params }) => (params.q === false ? 0 : 1) *
      (cons >= 1 ? talent[GsTalentType.q].get('火2伤害提升'] : talent[GsTalentType.q].get('火1伤害提升'])
  }
}, 'spread']
