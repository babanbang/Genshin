import { AttrKeys, MaterialType, WeaponDetailInfo } from "@/types";

/** 冲浪时光 */
export const metaData: WeaponDetailInfo = {
	desc: "原本预订发放给「第一届全部族冲浪竞速锦标赛」冠军的纪念品。鉴于赛事目前尚处于筹备阶段，奖品便被临时寄存在了「悠游豚豚」内，由店主代为保管。",
	attr: {
		atk: new Map([
			["1", 44.34],
			["20", 110.17],
			["40", 209.82],
			["50", 275.46],
			["60", 341.46],
			["70", 407.81],
			["80", 474.55],
			["90", 541.83],
			["20+", 141.27],
			["40+", 240.92],
			["50+", 306.66],
			["60+", 372.56],
			["70+", 438.91],
			["80+", 505.65]
		]),
		bonusKey: AttrKeys.cdmg,
		bonusData: new Map([
			["1", 19.2],
			["20", 33.93],
			["40", 49.44],
			["50", 57.2],
			["60", 64.95],
			["70", 72.69],
			["80", 80.45],
			["90", 88.2],
			["20+", 33.93],
			["40+", 49.44],
			["50+", 57.2],
			["60+", 64.95],
			["70+", 72.69],
			["80+", 80.45]
		])
	},
	materials: {
		[MaterialType.weapon]: "贡祭炽心的荣膺",
		[MaterialType.monster]: "意志巡游的符像",
		[MaterialType.normal]: "横行霸者的利齿"
	},
	affixData: {
		title: "水色回忆",
		desc: "生命值上限提高$[0]。每15秒一次，施放元素战技后的14秒内，产生如下效果：获得4层「炽夏」，每层使普通攻击造成的伤害提升$[1]。持续期间内，每1.5秒一次：普通攻击命中敌人后，移除1层；每1.5秒一次：对敌人触发蒸发反应后，增加1层。「炽夏」效果至多叠加4层。",
		datas: {
			"0": [
				"20%",
				"25%",
				"30%",
				"35%",
				"40%"
			],
		"1": [
				"12%",
				"15%",
				"18%",
				"21%",
				"24%"
			]
		}
	}
}